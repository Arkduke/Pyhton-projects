...
tk.Label(ra,text="File Address",fg='green', bg='black', font=('comicsans', 15)).grid(row=0)
tk.Label(ra,text="Number of Students",fg='green', bg='black', font=('comicsans', 15)).grid(row=1)
tk.Label(ra,text="Name of Output File",fg='green', bg='black', font=('comicsans', 15)).grid(row=2)
tk.Label(ra,text='''USER MANUAL:
...
2. Fill the absolute address of the input file provided with the software or if the file
       is present in the directory of the program mention the file name with the extension.
...
5. The blank places in the excel sheet should be filled with zero.
6. The total number of Students is limited to 100.
7. All fields are mandatory, after complete fillup click on get results.
...
tk.Button(ra, text='Get Results',command=ra.quit,font =('calibri', 10, 'bold'), foreground = 'green',height=3,width=79).grid(row=7,column=1)
def close(): 
    ra.destroy()
tk.Button(ra, text = "Quit", command = close,font =('calibri', 10, 'bold', 'underline'), foreground = 'red', height=1, width=6).grid(row=3,column=0)
tk.mainloop()
...
messagebox.showinfo("Message", '''The Program has been executed successfully!
Search for the created file in the base directory or the directory specified in the address bar.''')

# made my Arkduke(Arka Sarkar)
